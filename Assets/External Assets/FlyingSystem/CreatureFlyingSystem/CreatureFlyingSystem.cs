...
namespace FlyingSystem
{
    public class CreatureFlyingSystem : MonoBehaviour
    {
        ...
        public ShaderGlobalVariablesUpdater shaderUpdater;
        ...
        public float shaderAlpha = 0;
        public float shaderMask = 0.6f;
        ...
        void Start()
        {
            ...
            for (int i = 0; i < shaderUpdater.variables.Count; i++)
            {
                if (shaderUpdater.variables[i].globalReference == "_Alpha")
                {
                    shaderAlpha = shaderUpdater.variables[i].floatValue;
                }
                if (shaderUpdater.variables[i].globalReference == "_Mask_Size")
                {
                    shaderMask = shaderUpdater.variables[i].floatValue;
                }
            }
        }
        ...
        void Update()
        {
            if (enabledFlyingLogic)
            {
                ...
                for (int i = 0; i < shaderUpdater.variables.Count; i++)
                {
                    if (shaderUpdater.variables[i].globalReference == "_Alpha")
                    {
                        shaderUpdater.variables[i].floatValue = shaderAlpha;
                    }
                    if (shaderUpdater.variables[i].globalReference == "_Mask_Size")
                    {
                         shaderUpdater.variables[i].floatValue = shaderMask;
                    }
                }
            }
        }
